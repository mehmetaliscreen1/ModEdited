...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  if (!message.member.voice.channel || !uye.voice.channel || message.member.voice.channelID == uye.voice.channelID) return message.react(client.emojiler.iptal);// message.channel.send(embed.setDescription("Belirtilen üyenin ve kendinin ses kanalında olduğundan emin ol!")).then(x => x.delete({timeout: 5000}));
  if (message.member.hasPermission("ADMINISTRATOR")) {
    ...
    message.channel.send(`${uye}`, {embed: embed.setAuthor(uye.displayName, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${message.author} adlı kişi sizi kendi sesli odasına (\`${message.guild.cache.find(message.member.voice.channelID).name}\`) çekmek istiyor, onaylamak için tepkiye basmalısınız.`)}).then(async msj => {
      await msj.react('✅');
