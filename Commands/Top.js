...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  	let raw = db.get("raw");
	  let embed = new MessageEmbed().setAuthor(message.author.username, message.author.avatarURL({dynamic:true})).setColor("RANDOM").setFooter(conf.footer).setDescription(`${message.guild.name} sunucusunda son __14 günlük__ kullanıcı aktiflik verileri. \n Bu verilerin kayıt olma şekli Türkiye saati ile yapılmaktadır.`);
	  ...
	  embed.addField("Tek Günlük Mesajlar;", indaytext.join("\n") || "Hiçbir veri yok.");
	  embed.addField("1 Haftalık Mesajlar", `Toplam Mesaj: \`${inweektextTotal} mesaj\` \n\n ${inweektextObject.join("\n") || "Hiçbir veri yok."}`);
	  embed.addField("2 Haftalık Mesajlar", `Toplam Mesaj: \`${intwoweekTotal} mesaj\` \n\n ${intwoweektextObject.join("\n") || "Hiçbir veri yok."}`);
