...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.muteciRolleri.some(rol => message.member.roles.cache.has(rol)) && !message.member.roles.cache.has(ayar.sahipRolu)) return message.react(client.emojier.iptal); //message.channel.send(embed.setDescription(`Unmute komutunu kullanabilmek için herhangi bir yetkiye sahip değilsin.`)).then(x => x.delete({timeout: 5000}));
