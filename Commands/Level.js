    ...
    client.on("message", async message => {
      ...
      if (nextLevel <= currentlyData.XP) {
        ...
        message.channel.send(
          `${message.member}, **tebrikler!** Seviye atladın ve **${currentlyData.Level}. seviyeye** ulaştın. ${client.emojiler.onay}`
        );
      }
...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    let embed = new MessageEmbed().setThumbnail(user.user.avatarURL({dynamic: true, size: 2048})).setAuthor(user.displayName, user.user.avatarURL({dynamic: true, size: 2048})).setFooter(conf.footer).setColor(client.randomColor()).setTimestamp();
