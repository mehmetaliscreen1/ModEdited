...
module.exports.execute = async (client, message, args) => {
  ...
  let ozellikler = [
    ...
    { name: "komutCezasi", type: "rol" },
    { name: "punitiveKanali", type: "kanal" },
  ];
  let secim = args[0];
  const embed = new MessageEmbed().setColor(client.randomColor()).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setFooter(conf.footer);
  if (secim == "liste") {
  ...
  if (ozellik.type == "tekil"){
    ...
    return message.channel.send(embed.setDescription("<a:helios:775100486646890507> **Ayar başarıyla belirttiğin şekilde ayarlandı!**"));
  }
    ...
    return message.channel.send(embed.setDescription(`<a:helios:775100486646890507> Belirttiğin ayar için ${rol} tanımladın.`))
  }
    ...
    return message.channel.send(embed.setDescription(`<a:helios:775100486646890507> Belirttiğin ayar için ${channel} tanımladın.`));
  }
    ...
    return message.channel.send(embed.setDescription(`<a:helios:775100486646890507> Belirttiğin ayar tanımlandı. Mevcut ayarında \`${arr.join(", ")}\` bulunmakta.`));
  }
  else if (ozellik.type == "acmali"){
    await db.set(`ayar.${ozellik.name}`, !db.get(`ayar.${ozellik.name}`));
    return message.channel.send(embed.setDescription(`<a:helios:775100486646890507> Belirttiğin ayar ${db.get(`ayar.${ozellik.name}`) ? "**açıldı!**" : "**kapatıldı**!"}`));
  }
};
