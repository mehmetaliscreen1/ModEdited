...
client.on("userUpdate", async(oldCAD, newCAD) => {
  ...
  if((cadPP).endsWith(".gif")) {
    client.channels.cache.get(cadGIF).send(new MessageEmbed().setFooter(conf.durum).setColor(client.randomColor()).setImage(cadPP));
  } else {
    client.channels.cache.get(cadNORMAL).send(new MessageEmbed().setFooter(conf.durum).setColor(client.randomColor()).setImage(cadPP));
  };
});
...
if(message.member.hasPermission("ADMINISTRATOR")) return;
  if (!links) return;
  message.delete();
  message.channel.send(`<a:helios:772540868996759643> ${message.author}, bu sunuucuda link paylaşılması yasaklanmıştır.`)
});
...
client.on("message", async msg => {
  ...
  if (msg.content.includes() === (".gg") ||  msg.content.toLowerCase().startsWith("dc.") || msg.content.toLowerCase().startsWith("discord.") || msg.content.toLowerCase().startsWith("http") || msg.content.toLowerCase().startsWith("https") || msg.content.toLowerCase().startsWith("discord.gg"))   msg.delete() && msg.member.roles.add("772716662372827136");
});

client.on("message", async message => {
  if(message.member.roles.cache.has("772328674246262864")) return;
  if(message.member.id === "409792719037136918") return;
 let durum = ["@everyone", "@here","<@&772713449703735327>","<@&772713449888940053>","<@&772713450429480962>","<@&772713448282652672>","<@&772713448701558785>","<@&772713449163194370>"].some(küfür => message.content.includes(küfür));
if(durum) return message.channel.send(`${message.author} sunucumuzda toplu etiket atamazsınız.`) || message.delete() || message.member.roles.set(["772716576759087126"]);
