...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  const embed = new discord.MessageEmbed().setAuthor(user.displayName, user.user.avatarURL({dynamic: true})).setFooter(conf.footer).setColor(client.randomColor()).setTimestamp();
  ...
  if(newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(ayarlar.ekipRolu)){
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} adlı kullanıcı ailemize katıldı.`).setColor("#32FF00")).catch();
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} adlı kullanıcı ailemizden ayrıldı.`).setColor("#B20000")).catch();
  }
}
